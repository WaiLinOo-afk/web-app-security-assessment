...
Usage:
    ...
    3. Get your PHPSESSID from browser dev tools (F12 > Storage > Cookies)
    ...
    try:
        ...
        if not token_found:
            ...
            if r2.status_code == 200 and "login" not in r2.url.lower():
                log("  [VULN] CSRF likely -- form submitted without token, server returned 200", f)
                found.append(url)
            else:
...
def main():
    ...
    with open(OUTPUT_FILE, "w") as f:
        ...
        all_findings += [("SQLi", p) for p in test_sqli(f"{TARGET}/vulnerabilities/sqli/", "id", f)]
        all_findings += [("XSS", p) for p in test_xss(f"{TARGET}/vulnerabilities/xss_r/", "name", f)]
        all_findings += [("CMDI", p) for p in test_cmdi(f"{TARGET}/vulnerabilities/exec/", "ip", f)]
        all_findings += [("LFI", p) for p in test_lfi(f"{TARGET}/vulnerabilities/fi/", "page", f)]

        csrf_fields = {"password_new": "csrf_test", "password_conf": "csrf_test", "Change": "Change"}
        all_findings += [("CSRF", u) for u in test_csrf(f"{TARGET}/vulnerabilities/csrf/", csrf_fields, f)]
